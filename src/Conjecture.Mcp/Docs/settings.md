...
public sealed record ConjectureSettings
{
    ...
    public bool Database { get; init; }             // default: true (SQLite example cache)
    public TimeSpan? Deadline { get; init; }           // null = no per-example time limit
    public int MaxStrategyRejections { get; init; }    // default: 200
    public bool ExportReproductionOnFailure { get; init; }    // default: false; writes .cs repro file on failure
    public string ReproductionOutputPath { get; init; }       // default: ".conjecture/repros/"
}
```
...
Disable with `[ConjectureSettings(Database = false)]`.

## Reproduction Export

When `ExportReproductionOnFailure` is enabled, Conjecture writes a `.cs` file containing a deterministic `[Property(Seed = 0x...)]` snippet for each failing test:

```csharp
[ConjectureSettings(ExportReproductionOnFailure = true, ReproductionOutputPath = ".conjecture/repros/")]
[Property]
public void MyTest(int x) { ... }
```

On failure, a file is written to `ReproductionOutputPath` with the seed and minimal counterexample, ready to paste into a test class for deterministic replay.
