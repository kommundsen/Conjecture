...
## 2. Generate entities with `Strategy.Entity<T>`

Use `Strategy.Entity<T>(context)` to produce property-based strategies for EF Core entity types tracked by a `DbContext`:
...
## 3. Sample existing rows with `Strategy.EntitySet<T>`

Use `Strategy.EntitySet<T>(context)` to draw from rows already persisted in the database:
