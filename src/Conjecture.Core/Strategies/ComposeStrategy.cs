...
{
    private readonly ConjectureData _data;
    internal GeneratorContext(ConjectureData data) => _data = data;

    public T Next<T>(Strategy<T> strategy) => strategy.Next(_data);
...
internal sealed class ComposeStrategy<T> : Strategy<T>
{
    ...
    private readonly Func<IGeneratorContext, T> _factory;
    internal ComposeStrategy(Func<IGeneratorContext, T> factory) => _factory = factory;

    internal override T Next(ConjectureData data)
    {
        for (var i = 0; i < MaxAttempts; i++)
        {
            try { return _factory(new GeneratorContext(data)); }
