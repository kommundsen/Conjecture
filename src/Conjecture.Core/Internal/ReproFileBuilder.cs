...
internal static class ReproFileBuilder
{
    ...
    private static void AppendFileHeader(StringBuilder sb, ulong seed, DateTimeOffset generatedAt)
    {
        ...
        sb.AppendLine($"// Seed: 0x{seed:X}");
        sb.AppendLine($"// OS: {os}");
        sb.AppendLine($"// .NET: {dotnet}");
        sb.AppendLine($"// Arch: {arch}");
    }

    private static void AppendTestAttributes(StringBuilder sb, ulong seed, TestFramework framework)
    {
        sb.AppendLine($"[Property(Seed = 0x{seed:X})]");
    ...
    internal static string BuildStateMachine(StateMachineReproContext context)
    {
        StringBuilder sb = new();

        AppendFileHeader(sb, context.Seed, context.GeneratedAt);
        sb.AppendLine();
        sb.AppendLine(GetFrameworkUsing(context.Framework));
        sb.AppendLine();
        AppendTestAttributes(sb, context.Seed, context.Framework);
        sb.AppendLine("public void Counterexample()");
        sb.AppendLine("{");
        sb.AppendLine($"    {context.SutTypeName} sut = new();");

        foreach ((string label, object? state, Type stateType) in context.Commands)
        {
            string stateStr = RenderValue(state, stateType);
            sb.AppendLine($"    sut.{label}(); // State: {stateStr}");
        }

        if (context.ViolatedInvariant is not null)
        {
            sb.AppendLine($"    // Invariant violated: {context.ViolatedInvariant}");
        }
