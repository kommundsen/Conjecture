...
public sealed class AnalyzerIntegrationE2ETests
{
    ...
    private const string AllViolationsSource = """
        ...
        class Tests {
            ...
            void BadStrategy() { var s = Gen.Integers<int>().Where(x => x == 42); }
            ...
            void InvertedBounds() { var s = Gen.Integers(10, 5); }
    ...
    public async Task CON103CodeFix_SwapsToCorrectBounds()
    {
        ...
        Assert.NotNull(fixed_);
        Assert.Contains("Gen.Integers(5, 10)", fixed_);
    }
    ...
    public async Task CleanPropertyTest_ProducesNoDiagnostics()
    {
        string source = Preamble + """
            class Tests {
                ...
                {
                    var s = Gen.Lists(Gen.Integers<int>(), minSize: 0, maxSize: 10);
                    return true;
                }
            }
            """;
    ...
    public async Task ValidGenBounds_ProducesNoDiagnostics()
    {
        string source = Preamble + """
            class Tests {
                void Setup()
                {
                    var a = Gen.Integers(0, 100);
                    var b = Gen.Doubles(0.0, 1.0);
                    var c = Gen.Strings(minLength: 0, maxLength: 20);
                }
            }
            """;
    ...
    public async Task GeneratedCodeAnnotated_AnalyzersSkipGeneratedCode()
    {
        ...
        string source = """
            ...
            class GeneratedTests {
                void M() { var s = Gen.Integers(10, 5); }
            }
            """;
