...
public static class Gen
{
    ...
    /// <summary>Returns a strategy that generates random <see cref="List{T}"/> with length in [<paramref name="minLength"/>, <paramref name="maxLength"/>].</summary>
    public static Strategy<List<T>> ListOf<T>(Strategy<T> elementStrategy, int minLength = 0, int maxLength = 10)
        => new ListStrategy<T>(elementStrategy, minLength, maxLength);
}
