...
Both samples produce the same shape: build an in-process minimal-API host, extract `IHost` + `HttpClient`, build a `Strategy<HttpInteraction>` via `Generate.AspNetCoreRequests`, then assert the never-5xx invariant via `Property.ForAll`. The fixture lifetime is the only thing that differs from a test-framework example.
