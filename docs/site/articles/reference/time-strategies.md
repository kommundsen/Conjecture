...
> [!NOTE]
> Requires the `Conjecture.Time` NuGet package. The core `Strategy.DateTimeOffsets()`, `Strategy.TimeSpans()`, `Strategy.DateOnlyValues()`, `Strategy.TimeOnlyValues()`, and `Strategy.DateTimes()` methods are in `Conjecture.Core` and do not require this package.
...
```csharp
Strategy<TimeZoneInfo> Strategy.TimeZones()
```

Picks uniformly from all system time zones via `TimeZoneInfo.GetSystemTimeZones()`. Shrinks toward `TimeZoneInfo.Utc`.

> [!NOTE]
> Available IDs differ across operating systems. For reproducible cross-platform tests, prefer `Strategy.TimeZone()` or `Strategy.IanaZoneIds()`.

### `Strategy.TimeZone(bool preferDst = false)`

```csharp
Strategy<TimeZoneInfo> Strategy.TimeZone(bool preferDst = false)
```
...
### `Strategy.IanaZoneIds(bool preferDst = false)`

```csharp
Strategy<string> Strategy.IanaZoneIds(bool preferDst = false)
```

Returns IANA timezone IDs from the same cross-platform-safe curated subset as `Strategy.TimeZone()`. Pass `preferDst: true` to restrict to DST-having zones. Use the returned string with `TimeZoneInfo.FindSystemTimeZoneById(id)`.

### `Strategy.WindowsZoneIds()`

```csharp
Strategy<string> Strategy.WindowsZoneIds()
```
...
### `Strategy.ClockSet(int nodeCount, TimeSpan maxSkew)`

```csharp
Strategy<FakeTimeProvider[]> Strategy.ClockSet(int nodeCount, TimeSpan maxSkew)
```
...
### `Strategy.AdvancingClocks(TimeSpan maxJump)`

```csharp
Strategy<FakeTimeProvider> Strategy.AdvancingClocks(TimeSpan maxJump)
```
...
### `Strategy.ClockWithAdvances(int advanceCount, TimeSpan maxJump, bool allowBackward = false)`
...
### `Strategy.RecurringEvents(...)`

```csharp
Strategy<RecurringEventSample> Strategy.RecurringEvents(
...
Chain after `Strategy.DateTimeOffsets()`. These methods are in `Conjecture.Core` except where noted.
...
Chain after `Strategy.DateOnlyValues()`.
...
Chain after `Strategy.TimeOnlyValues()`.
...
Chain after `Strategy.DateTimes()`.
